...
[scode type="yellow"]注意: 为了兼顾大多数用户,本教程使用远程SSH连接Debian服务器进行讲解,同时会给出MacOS的命令,windows本地开发者请自行安装wsl2作为替代 [/scode]
...
-   配置[vscode](https://code.visualstudio.com/)实现调试与在保存代码时自动运行`eslint`/
